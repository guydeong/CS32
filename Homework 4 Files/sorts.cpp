...
int main()
{
    ...
    if (TEST_INSERTION_SORT)
    {
        ...
        report("Trying to implement", timer.elapsed(), stores);
    ...
    assert(is_sorted
    (stores.begin(), stores.end(), compareStore) && allPresent(stores));
}
...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
    ...
    for (int i = 0; i < stores.size(); i++) {
        swap(stores[i], *auxStoresUsingPtrs[i]);
    }
